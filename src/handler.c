...
char *weapon_bit_name(int weapon_flags)
{
    ...
    if (weapon_flags & WEAPON_ENERGYDRAIN ) strcat(buf, " energydrain");  
...
if (is_affected(ch,gsn_shadow_dance) || ch->morph_form[0] == MORPH_CONCEAL3)
    {
//     return "a cloaked figure";
     return get_descr_form(ch,looker,FALSE);
    }
...
char *room_affect_name(int room_flags)
{
    ...
    if (room_flags & ROOM_AFF_OVERGROWN		) strcat(buf, " overgrown");
