...
void fire_effect(void *vo, int level, int dam, int target)
{
        ...
        if(!is_affected(victim,gsn_on_fire))
        {
         if(!saves_spell(level , victim, DAM_FIRE))
          if(number_percent() > 70)
          {
            ...
            act("Your skin starts to burn hotly!",victim,NULL,NULL,TO_CHAR);
	...
	/* chance of blindness */
        else if (!IS_AFFECTED(victim,AFF_BLIND) && !saves_spell(level, victim,DAM_FIRE) && (number_range(1,10)< 7))
	{
            AFFECT_DATA af;
...
void shock_effect(void *vo,int level, int dam, int target)
{
        ...
        if (number_range(1,6)==number_range(1,7))
	{
	    ...
	    act("$n's body goes stiff with paralysis!",victim,NULL,NULL,TO_ROOM);
