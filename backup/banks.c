...
void do_deposit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( ch->balance[bank] + amount ) > 40000 )
    {
        sprintf(buf,
            "%s tells you '{_Sorry, but I can only hold 40,000 coins per player.{x'\n\r",
            mob->short_descr);
        send_to_char( buf, ch);
        return;
    }
...
void do_withdraw( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "Withdrawals must be made in multiples of 50 platinum.\n\r", ch);
        return;
    }
