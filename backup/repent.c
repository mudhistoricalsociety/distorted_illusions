...
void do_repent(CHAR_DATA *ch, char *argument)
{
	...
	send_to_char("  align: {&O{7di{&n{x's forgiveness     20 gold\n\r",ch);
	send_to_char("  bless: {&O{7di{&n{x's blessing        20 gold\n\r",ch);
    ...
    {
        act("$N says '{_Type 'repent' for a list of spells.{x'",
	    ch,NULL,mob,TO_CHAR);
	return;
    }
	...
	send_to_char("You feel {!H{1e{!l{x's anger toward your actions!\n\r",ch);
	return;
     }
