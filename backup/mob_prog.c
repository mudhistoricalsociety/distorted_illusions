    ...
    while ( *str != '\0' )
    {
		...
		if ( ch != NULL && can_see( mob, ch ) )
		{
	    	...
	    	i = (ch != NULL && can_see( mob, ch ) )
                ? ( IS_NPC( ch ) ? ch->short_descr : get_pers(ch,NULL) )
		: someone;                         		break;
            case 't': 
		i = someone;
		if ( vch != NULL && can_see( mob, vch ) )
		{
	    	...
	    	i = (vch != NULL && can_see( mob, vch ))
                ? ( IS_NPC( vch ) ? vch->short_descr : get_pers(vch,NULL) )
		: someone;                         		break;
            case 'r': 
		if ( rch == NULL ) 
	    ...
	    case 'Q':
	    	i = (mob->mprog_target != NULL && can_see( mob, mob->mprog_target ))
                ? ( IS_NPC( mob->mprog_target ) ? mob->mprog_target->short_descr : get_pers(mob->mprog_target,NULL) )
		: someone;                         		break;
	...
	const void *arg1, const void *arg2, int type )
{
	...
	&&  strcasestr( argument, prg->trig_phrase ) != NULL )
        {
