...
*/
    ...
    buffer = new_buf();
    sprintf(buf,"{r  ___________________________________________________________________________{x\n\r");
    add_buf(buffer,buf);
    sprintf(buf,"{r /\\_\\{x%70s{r\\_\\{x\n\r", " ");
    add_buf(buffer,buf);

    add_buf( buffer, "{r|/\\\\_\\                  {wThe Immortals of {$D{&i{8s{$t{&o{8r{$t{&e{8d {wIllusions                {r\\_\\{x\n\r" );

    sprintf(buf,"{r\\_/_|_|{x%69s{r|_|{x\n\r", " ");
    add_buf(buffer,buf);
	...
	if (!found)
	{
	    if (level == HERO+1)
	    {
                sprintf(buf,"{r ___|_|{x%69s{r|_|{x\n\r", " ");
		add_buf(buffer,buf);
	    }
	    continue;
	}
        sprintf(buf,"{r    |_|{R%37s {B[%d]{x%26s{r|_|{x\n\r",
	    wiz_titles[IMPLEMENTOR-level], level, " ");
	add_buf(buffer,buf);
        sprintf(buf,"{r    |_|{Y%25s******************{x%26s{r|_|{x\n\r",
	    " ", " ");
	add_buf(buffer,buf);
	lngth = 0;
        for (pwiz = wiz_list;pwiz != NULL;pwiz = pwiz->next)
		...
		if (lngth == 0)
		{
		    ...
		    {
                        sprintf(buf, "{r    |_|{x{%s%30s%-39s{r|_|{x\n\r",
			    level >= DEMI ? "G" : "C", " ",
			    pwiz->name);
			add_buf(buffer, buf);
			lngth = 0;
    ...
    sprintf(buf,"{r|\\//_/{x%70s{r/_/{x\n\r", " ");
    add_buf(buffer,buf);
    sprintf(buf,"{r \\/_/______________________________________________________________________/_/{x\n\r");
    add_buf(buffer,buf);
